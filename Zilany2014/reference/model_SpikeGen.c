/* 
    This script contains a standalone version of the spike generator model included in the Zilany et al. ANF model.
    The mean firing rate is given as an input and the PSTH response is simulated as the output.
    Adapted by Fotios Drakopoulos, Ghent University, October 2020
/*
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double cf, tdres, fibertype, noiseType, implnt;
	int    nrep, pxbins, lp, totalstim;
    mwSize  outsize[2];

	double *cftmp, *nreptmp, *tdrestmp, *fibertypetmp, *noiseTypetmp, *implnttmp;
        
    double *meanrate, *psth;
	...
	SingleAN(meanrate,cf,nrep,tdres,totalstim,fibertype,noiseType,implnt,psth);


}

void SingleAN(double *meanrate, double cf, int nrep, double tdres, int totalstim, double fibertype, double noiseType, double implnt, double *psth)
{	
    ...
    /* repeat the stimulus nrep times */
    //synouttmp = repeat 
    for(i = 0; i<I ; i++)
	{       
		ipst = (int) (fmod(i,totalstim));
        synouttmp[i] = meanrate[ipst];
	};
